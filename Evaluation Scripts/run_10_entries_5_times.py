...
for wh in range(4):
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-topic2-{wh+2}": result}},
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-type_of_evaluation2-{wh+2}": result}},
            upsert=True  # Create the field if it doesn't exist
        )
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-target_user_group2-{wh+2}": result}},
            upsert=True  # Create the field if it doesn't exist
        )
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-research_problem2-{wh+2}": result}},
            upsert=True  # Create the field if it doesn't exist
        )
        print(f"ResearchProblem updated for entry ID: {entry_id}")
    print(f'Invalid Results: {invalid_results}')

    openess = mapping.get('Openness', None)
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-openess2-{wh+2}": result}},
            upsert=True  # Create the field if it doesn't exist
        )
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-modelling_approach2-{wh+2}": result}},
            upsert=True  # Create the field if it doesn't exist
        )
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-improvement_proposal2-{wh+2}": result}},
            upsert=True  # Create the field if it doesn't exist
        )
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-ethics2-{wh+2}": result}},
            upsert=True  # Create the field if it doesn't exist
        )
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-data_used2-{wh+2}": result}},
            upsert=True  # Create the field if it doesn't exist
        )
    ...
    for i, entry in enumerate(filtered_data):
        ...
        collection.update_one(
            {"_id": entry_id},
            {"$set": {f"como-tax-contribution2-{wh+2}": result}},
            upsert=True  # Create the field if it doesn't exist
        )
